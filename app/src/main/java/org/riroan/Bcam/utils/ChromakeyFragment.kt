...
import org.riroan.Bcam.filter.SegmentationAnalyzer
...
class ChromakeyFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_chromakey, container, false)
    }
    ...
    class Adapter(context: Context) : RecyclerView.Adapter<Adapter.ViewHolder>() {
        ...
        var itemClickListener = object : OnItemClickListener {
            override fun onItemClick(view: View, position: Int) {
                when (items[position]) {
                    ...
                    else -> MainActivity.imageProcessor = SegmentationAnalyzer(items[position])
                }
            }
        }
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            holder.itemView.findViewById<TextView>(R.id.filter_name).text = "Chromakey ${position + 1}"
            if (position == 0) {
    ...
    companion object {
        private val items = arrayOf(-1, R.raw.back1, R.raw.back4)
