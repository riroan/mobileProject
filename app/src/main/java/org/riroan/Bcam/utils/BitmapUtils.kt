...
object BitmapUtils {
    ...
    private fun getExifOrientationTag(resolver: ContentResolver, imageUri: Uri): Int {
        ...
        if (ContentResolver.SCHEME_CONTENT != imageUri.getScheme()
            && ContentResolver.SCHEME_FILE != imageUri.getScheme()
        ) {
            return 0
        }
    ...
    fun convertBitmapToNv21Bytes(bitmap: Bitmap): ByteArray {
        ...
        val nv21Bytes = ByteArray(
            inputHeight * inputWidth
                    + 2 * Math.ceil(inputHeight / 2.0).toInt() * Math.ceil(inputWidth / 2.0)
                .toInt()
        )
    ...
    private fun encodeToNv21(nv21Bytes: ByteArray, argb: IntArray, width: Int, height: Int) {
        ...
        for (j in 0 until height) {
            for (i in 0 until width) {
                ...
                nv21Bytes[yIndex++] = (if ((y < 0)) 0 else min(255, y)).toByte()
                if (j % 2 == 0 && index % 2 == 0) {
                    nv21Bytes[uvIndex++] = (if ((v < 0)) 0 else min(255, v)).toByte()
                    nv21Bytes[uvIndex++] = (if ((u < 0)) 0 else min(255, u)).toByte()
                }
                index++
            }
        }
    ...
    private fun yuv420ThreePlanesToNV21(
        ...
        if (areUVPlanesNV21(yuv420888planes, width, height)) {
            ...
            val uBuffer: ByteBuffer = yuv420888planes[1].buffer
            val vBuffer: ByteBuffer = yuv420888planes[2].buffer
